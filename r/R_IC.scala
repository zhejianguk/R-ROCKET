...
class R_IC (val params: R_ICParams) extends Module with HasR_ICIO {
  ...
  switch (fsm_state) {
    ...
    is (fsm_presch){ // 001
      ...
      if_filtering                              := 0.U
      if_pipeline_stall                         := Mux(fsm_ini.asBool, 0.U, Mux((exception.asBool || (!io.if_correct_process.asBool)), 0.U, 1.U))
    ...
    is (fsm_check){ // 110
      ...
      ctrl                                      := Mux(io.ic_exit_isax.asBool, 3.U, Mux(io.ic_syscall.asBool || if_t_and_na.asBool, 1.U, Mux(if_t_and_a.asBool, 1.U, ctrl)))
      crnt_target                               := crnt_target
      crnt_mask                                 := crnt_mask
      nxt_target                                := nxt_target 
