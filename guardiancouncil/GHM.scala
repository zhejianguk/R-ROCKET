...
class GHM (val params: GHMParams)(implicit p: Parameters) extends LazyModule
{
  lazy val module = new LazyModuleImp(this) {
    ...
    /* val u_cdc                                      = Seq.fill(params.number_of_little_cores) {Module(new GH_CDCH2LFIFO_HandShake(GH_CDCH2L_Params (0, params.width_GH_packet, 40)))} */

    val firstCDC = Module(new GH_CDCH2LFIFO_HandShake(GH_CDCH2L_Params(0, params.width_GH_packet, 4)))
    val remainingCDCs = Seq.fill(params.number_of_little_cores - 1) {
      Module(new GH_CDCH2LFIFO_HandShake(GH_CDCH2L_Params(0, params.width_GH_packet, 40)))
    }
    val u_cdc = firstCDC +: remainingCDCs
